...
export const fetchWeeklyProgress = async (): Promise<
...
export const fetchDateProgress = async (
  date: string
): Promise<DailyProgressWeekItem> => {
  try {
    const token = await AsyncStorage.getItem(ACCESS_TOKEN_KEY);
    const url = `${API_BASE_URL}/api/daily-progress/date?date=${encodeURIComponent(
      date
    )}`;
    const response = await axios.get(url, {
      headers: {
