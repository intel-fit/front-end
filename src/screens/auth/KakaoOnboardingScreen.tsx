...
const KakaoOnboardingScreen = ({ navigation }: any) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const healthGoalValue = ["FLEXIBILITY", "ENDURANCE", "POSTURE"].includes(
        formData.healthGoal
      )
        ? "MAINTENANCE"
        : formData.healthGoal;
          ...
          <View style={styles.inputGroup}>
            <TouchableOpacity
              ...
              onPress={() => {
                const currentYear = new Date().getFullYear();
                const defaultYear =
                  formData.birthYear || String(currentYear - 20);
                ...
                value={
                  formData.birthYear && formData.birthMonth && formData.birthDay
                    ? `${formData.birthYear}-${String(
                        formData.birthMonth
                      ).padStart(2, "0")}-${String(formData.birthDay).padStart(
                        2,
                        "0"
                      )}`
                    : ""
          ...
          <View style={styles.inputGroup}>
            ...
            >
              <TextInput
                ...
                value={
                  formData.healthGoal
                    ? healthGoalOptions.find(
                        (opt) => opt.value === formData.healthGoal
                      )?.label || ""
                    : ""
                }
                placeholder="헬스 목적"
              ...
              <TextInput
                ...
                value={
                  formData.workoutDaysPerWeek
                    ? workoutDaysOptions.find(
                        (opt) => opt.value === formData.workoutDaysPerWeek
                      )?.label || ""
                    : ""
                }
