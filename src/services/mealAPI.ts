...
export const mealAPI = {
  ...
  getDailyMeals: async (date: string): Promise<DailyMealsResponse> => {
    ...
    const url = `${AI_API_BASE_URL}/food/get_meals?user_id=${encodeURIComponent(
      user_id
    ...
    if (!response.ok) {
      ...
      if (response.status === 422) {
        try {
          ...
          errorMessage =
            "요청 파라미터가 올바르지 않습니다. user_id와 date를 확인해주세요.";
        }
      } else {
        // 다른 에러 처리
        try {
    ...
    const dailyMeals: DailyMealsResponse = {
      ...
      dailyTotalCalories: meals.reduce(
        (sum: number, meal: any) =>
          sum +
          (meal.items?.reduce(
            (s: number, item: any) => s + (item.calories || 0),
            0
          ) || 0),
        0
      ),
      dailyTotalCarbs: meals.reduce(
        (sum: number, meal: any) =>
          sum +
          (meal.items?.reduce(
            (s: number, item: any) => s + (item.carbs || 0),
            0
          ) || 0),
        0
      ),
      dailyTotalProtein: meals.reduce(
        (sum: number, meal: any) =>
          sum +
          (meal.items?.reduce(
            (s: number, item: any) => s + (item.protein || 0),
            0
          ) || 0),
        0
      ),
      dailyTotalFat: meals.reduce(
        (sum: number, meal: any) =>
          sum +
          (meal.items?.reduce(
            (s: number, item: any) => s + (item.fat || 0),
            0
          ) || 0),
        0
      ),
  ...
  addMeal: async (mealData: AddMealRequest): Promise<AddMealResponse> => {
    ...
    const mealName =
      mealNameMap[mealData.mealType] || mealData.mealType.toLowerCase();
  ...
  deleteMeal: async (
    ...
    const url = `${AI_API_BASE_URL}/food/delete_meal?meal_id=${mealId}&user_id=${encodeURIComponent(
      user_id
    )}`;
  ...
  deleteMealItem: async (meal_item_id: number): Promise<any> => {
    ...
    const url = `${AI_API_BASE_URL}/food/delete_meal_item?meal_item_id=${meal_item_id}&user_id=${encodeURIComponent(
      user_id
    )}`;
  ...
  updateMealItem: async (
    ...
    const url = `${AI_API_BASE_URL}/food/update_meal_item?meal_item_id=${meal_item_id}&user_id=${encodeURIComponent(
      user_id
    )}`;
  ...
  getDailyGoal: async (): Promise<any> => {
    ...
    const url = `${AI_API_BASE_URL}/food/daily_goal?user_id=${encodeURIComponent(
      user_id
    )}`;
  ...
  getNutritionGoal: async (): Promise<NutritionGoal> => {
    ...
    const url = `${AI_API_BASE_URL}/food/nutrition-goal/get?user_id=${encodeURIComponent(
      user_id
    )}&date_str=${encodeURIComponent(today)}`;
    ...
    try {
      ...
      return {
        ...
        goalTypeDescription:
          goal.goal_type_description || goal.goalTypeDescription || "자동 계산",
      };
    } catch (error: any) {
      // 404 에러인 경우 기본값 반환
      if (
   ...
   */
  setNutritionGoal: async (
    goalData: SetNutritionGoalRequest
  ): Promise<SetNutritionGoalResponse> => {
    const user_id = await getUserId();
    const today = new Date().toISOString().split("T")[0];
  ...
  searchFood: async (name: string): Promise<SearchFoodResponse[]> => {
    ...
    const url = `${AI_API_BASE_URL}/food/search?name=${encodeURIComponent(
      name
    )}`;
  ...
  uploadFood: async (imageUri: string): Promise<any> => {
    ...
    try {
      ...
      const cleanFilename = `photo.${finalExtension}`.replace(
        /[^a-zA-Z0-9._-]/g,
        "_"
      );
      ...
      if (!response.ok) {
        ...
        // 422 Validation Error 처리
          ...
          if (errorData.detail) {
            ...
            errorMessage =
              "업로드한 파일이 올바르지 않습니다. 이미지 파일을 확인해주세요.";
          }
        } else if (errorData.message) {
          errorMessage = errorData.message;
        }
      ...
      const isImageManipulatorError =
        error.message && error.message.includes("ImageManipulator");
      const is400Error =
        error.message &&
      ...
      if (isImageManipulatorError || is400Error) {
        console.warn(
          "이미지 리사이즈 실패 또는 400 에러, 원본 이미지로 재시도"
        );
        try {
          const formData = new FormData();
          const originalFilename = imageUri.split("/").pop() || "photo.jpg";
          ...
          const cleanFilename =
            originalFilename.replace(/[^a-zA-Z0-9._-]/g, "_") || "photo.jpg";
