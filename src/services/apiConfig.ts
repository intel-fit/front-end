...
export const AI_API_BASE_URL = "https://intelfits.com/ai"; // AI 서버 주소
...
export const requestAI = async <T = any>(
  ...
  try {
    console.log("🤖 AI 서버 요청:", `${AI_API_BASE_URL}${endpoint}`);
    console.log("요청 옵션:", { method: options.method || "GET", headers });
    ...
    if (error.name === "AbortError") {
      console.error("⏱️ AI 서버 타임아웃:", `${timeoutMs}ms`);
    ...
    if (
      ...
      console.error("네트워크 연결 실패:", `${AI_API_BASE_URL}${endpoint}`);
      const networkError: any = new Error(
        "AI 서버에 연결할 수 없습니다. 네트워크 연결을 확인해주세요."
      );
      networkError.status = 0;
      throw networkError;
    }
